...
pub fn add_gun_should_return_error_when_gun_name_is_exist() {
    ...
    let name = "knife";
    assert!(guns
        .create_gun(name.to_string(), 100, 20, 100, TypeOfGun::Knife)
        .is_ok());
    assert!(guns
        .create_gun(name.to_string(), 10, 2, 500, TypeOfGun::Knife)
        .is_err());
}
...
pub fn add_gun_should_add_a_gun_in_list_of_gun() {
    ...
    assert!(guns
        .create_gun(name.to_string(), price, damage, gift, type_of)
        .is_ok());
...
pub fn get_knife_when_has_not_knife_shold_be_retur_error() {
    let mut guns = Guns::new();
    guns.create_gun("not a knife".to_string(), 100, 10, 10, TypeOfGun::Heavy)
        .unwrap();
...
pub fn get_knife_when_has_knife_shold_be_retur_knife() {
    ...
    let gift = 10;
    let type_of = TypeOfGun::Knife;
    guns.create_gun(name.to_string(), price, damage, gift, type_of)
        .unwrap();
...
pub fn get_gun_func_when_get_a_name_of_gun_does_not_exist_should_be_return_error() {
    ...
    guns.create_gun(name.to_string(), 100, 10, 10, TypeOfGun::Heavy)
        .unwrap();
...
pub fn get_guns_type_func_when_get_a_type_of_gun_should_be_return_list_of_gun_with_this_type() {
    ...
    guns.create_gun("gun 1".to_string(), 100, 10, 20, type_of)
        .unwrap();
    guns.create_gun("gun 2".to_string(), 100, 10, 20, type_of)
        .unwrap();
    guns.create_gun("gun 3".to_string(), 100, 10, 20, type_of)
        .unwrap();
    guns.create_gun("gun with other type".to_string(), 50, 10, 20, other_type_of)
        .unwrap();
