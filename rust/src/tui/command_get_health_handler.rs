...
pub struct CommandGetHealthHandler {
    name: String,
    status: State,
    time: String,
}

impl CommandGetHealthHandler {
    pub fn default() -> Self {
        Self {
            name: "".to_string(),
...
impl GameCommandHandler for CommandGetHealthHandler {
    fn run(&mut self, frame: &mut Frame, rect: Rect, game: &mut Game) -> Option<Log> {
        ...
        let block = Block::default()
            .borders(Borders::ALL)
            .title(Line::from(" Get Health Of Player ").right_aligned())
