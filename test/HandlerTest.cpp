...
protected:
    void SetUp() override {
        ...
        for (int i = 1; i < Setting::get_max_member_team(); ++i) {
            handler->add_user("T" + to_string(i), Gun::type_team::Terrorist,
                              "00:00:" + to_string_three_char(i));

            handler->add_user("CT" + to_string(i), Gun::type_team::Counter_Terrorist,
                              "00:00:" + to_string_three_char(i));
        }
    }
...
TEST_F(HandlerTest, AddUser) {
    ASSERT_NO_THROW(handler->add_user("T0", Gun::type_team::Terrorist, "00:00:001"));

    ASSERT_THAT(handler->add_user("CT0", Gun::type_team::Counter_Terrorist, "00:00:001"),
                Eq("this user added to Counter-Terrorist"));

    ASSERT_ANY_THROW(handler->add_user("T0", Gun::type_team::Counter_Terrorist, "00:00:001"));
    ASSERT_ANY_THROW(handler->add_user("T" + std::to_string(Setting::get_max_member_team() + 1),
                                       Gun::type_team::Terrorist, "00:00:001"));
}
...
TEST_F(HandlerTest, Tap) {
    EXPECT_NO_THROW(this->handler->tap("T1", "CT1", Gun::type_gun::knife));
    EXPECT_THAT(this->handler->tap("T1", "CT2", Gun::type_gun::knife), Eq("nice shot"));
    EXPECT_ANY_THROW(this->handler->tap("T1", "CT1", Gun::type_gun::heavy));
    EXPECT_ANY_THROW(this->handler->tap("T1", "T2", Gun::type_gun::knife));
    EXPECT_ANY_THROW(this->handler->tap("CT0", "CT1", Gun::type_gun::knife));
    EXPECT_ANY_THROW(this->handler->tap("T", "CT1", Gun::type_gun::knife));

    for (int i = 0; i < 100 / Setting::get_start_gun()->get_health(); ++i)
        this->handler->tap("T1", "CT1", Gun::type_gun::knife);

    EXPECT_ANY_THROW(this->handler->tap("CT1", "T1", Gun::type_gun::knife));
}
...
TEST_F(HandlerTest, Buy) {
    ...
    for (int i = 0; i <= 100 / Guns::get_gun("Revolver", Gun::access_level::terrorist)->get_health(); ++i)
        this->handler->tap("T1", "CT1", Gun::type_gun::pistol);
...
TEST_F(HandlerTest, NewRound) {
    ...
    for (int i = 1; i < Setting::get_max_member_team(); ++i)
        for (int j = 0; j <= 100 / Setting::get_start_gun()->get_health(); ++j)
            this->handler->tap("T1", "CT" + to_string(i), Gun::type_gun::knife);
